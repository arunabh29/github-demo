...
for arg in sys.argv:
    ...
    if keys[0] == 'workingLoc':
        ...
        earDirNameSet = True
        continue
    elif keys[0] == 'appPrefix':
        appPrefix = keys[1]
        appPrefixSet = True
        continue
    elif keys[0] == 'gigaspacesVersion':
        gigaspacesVersion = keys[1]
        gigaspacesVersionSet = True
        continue
    elif keys[0] == 'jdkVersion':
        jdkVersion = keys[1]
        jdkVersionSet = True
        continue
...
if projectNameFound == False or (projectNameFound == True and projectName == ['']) or (projectNameFound == True and projectName == ['PROJECT_TO_BUILD']):
    ...
    gsModuleName = projectName
    projectNames = projectTypes.findAllProjectNames(workingLoc, projectName.split(","))

if gigaspacesVersionSet == False:
    print "gigaspaces version must be provided"
    print "Please enter gigaspaces version as gigaspacesVersion="
    gigaspacesVersion = 9.6
    # sys.exit(1)
...
if appPrefixSet == False:
    print "AppPrefix name not provided - must be the application's EAR name"
    print "Please enter directory name as appPrefix="
    sys.exit(1)

if jdkVersionSet == False:
    print "jdk version not set, will use default version of jdk 1.7"
    jdkVersion = '7'
...
print ("jdkVersion found ======== " + jdkVersion + " ========")
...
antParseGigaspaces.parse(CSX_SCRIPTS_HOME, workingLoc, sourcePath, projectType, projectDependency, buildDir, earDirName, appPrefix, gigaspacesVersion, gsModuleName, jarFileName, buildSequence)
