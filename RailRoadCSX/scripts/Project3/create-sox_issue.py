...
# Script Name    : create_sox_issue.py
# Author         : Arunabh Chowdhury (Service Management Team)
# Creation Date  : 03/27/2016
# Description    : This script creates a JIRA issue for a SOX deployment and attaches the change-set file in the attachment field.
# Usage          : It is automatically triggered from inside a Bamboo build plan on a successful SOX production build.
# Modifications  :
# Ver	Name	   : 1.0
...
##########################################################################################################################
######  THIS SECTION CREATES JIRA ISSUE FOR A SOX PRODUCTION IMPLEMENTATION AND UPLOADS A CHANGE-SET ATTACHMENT FILE #####
##########################################################################################################################


# JIRA NON-PROD LOGIN
# options={'server': 'http://lnx21256:8080'}
# options={'server': 'https://jira-dev.csx.com', 'verify': False}
...
options={'server': 'https://jira.csx.com', 'verify': False}
...
jira_attachment_path=sys.argv[5]
...
print "******************************************************"
print "Uploading attachment for issue: %s" % issue_obj.key
# change_set_file = open('/home/t8054/Arunabh_sandbox/python_scripts/SOX/jira_attachment.txt', 'rb')
change_set_file = open(jira_attachment_path, 'rb')
jira.add_attachment(issue_obj, change_set_file)

print "******************************************************"

 
###############################################################################################################################
##########  DONE CREATING JIRA ISSUE FOR A SOX PRODUCTION IMPLEMENTATION AND UPLOADING A CHANGE-SET ATTACHMENT FILE ###########
###############################################################################################################################
...
tree.write(open(r'/home/t8054/Arunabh_sandbox/python_scripts/SOX/SM_Input_new.xml', 'w'))
