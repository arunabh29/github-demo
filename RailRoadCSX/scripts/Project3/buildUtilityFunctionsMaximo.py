...
#------------------------------------------------------------
# Parse the classpath sequence 
#------------------------------------------------------------

def parseClasspathSeq(classpathSequence):
...
#         sys.exit()
    ...
    customClasspathSequence = parseClasspathSeq(classpathSequence)
...
def cleanupBuildDir( aDir ) :
    top=aDir
    print 'Root of Build directory to be cleaned ' + top
    for root, dirs, files in os.walk(top, topdown=False):
        for name in files:
            #print 'file name  being removed ' + name
            os.remove(os.path.join(root, name))
        for name in dirs:
            #print 'Dir name being removed ' + name
...
def createFullBuildCommandArgs (currentBuildModule, buildDir, projectTypes, projectDependency, websphereLocFromEnv, earDir, earFileName, libDir, customClasspathSequence, excludeStarTestJava):
...
def resetProjList(workingLoc, earProjectPath, projectsListIn, ext):
    projectsList = list(projectsListIn)
    for project in projectsListIn:
        isDirectory = os.path.isdir(os.path.join(workingLoc, project))
        print 'dir ' + os.path.join(workingLoc, project) + ' is directory : ' + str(isDirectory)
        if (isDirectory == False):
            fileToCheck = project + ext
            isExtFile = os.path.isfile(os.path.join(earProjectPath, fileToCheck))
            print 'fileToCheck ' + fileToCheck + ' isExtFile ' + str(isExtFile)
            if (isExtFile == True):
                projectsList.remove(project)
    print ' projects ProjectsIn : ' + str(projectsListIn)
    print ' projects Projects : ' + str(projectsList)
    return projectsList

#--------------------------------------------------------------------
# Check if the Web / EJB projects exist as projects in the directory
# Or if they exist as WAR / jar files in the EAR
# If they exist as directory, then they should be built
# If they exist as WAR / Jar, then should be removed from build list
#--------------------------------------------------------------------
def cleanUpProjectList(workingLoc, projectTypesAllDict):
    earProjectPath = os.path.join(workingLoc, projectTypesAllDict['ear'][0])
    webProjects = resetProjList(workingLoc, earProjectPath, projectTypesAllDict['web'], '.war')
    ejbProjects = resetProjList(workingLoc, earProjectPath, projectTypesAllDict['ejb'], '.jar')
    utilProjects = resetProjList(workingLoc, earProjectPath, projectTypesAllDict['util'], '.jar')
    
    projectTypesAllDictReturn = {}
    
    projectTypesAllDictReturn['web'] = webProjects
    projectTypesAllDictReturn['ejb'] = ejbProjects
    projectTypesAllDictReturn['util'] = utilProjects
    projectTypesAllDictReturn['ear'] = list(projectTypesAllDict['ear'])

    return projectTypesAllDictReturn